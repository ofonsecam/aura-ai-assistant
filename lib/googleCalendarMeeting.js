...
/** Cuenta personal Gmail (referencia; no es el calendarId por defecto). */
const PERSONAL_GOOGLE_ACCOUNT = "fonseca.moreno.oscareduardo@gmail.com";
/** Calendario secundario personal: fallback si no hay GOOGLE_CALENDAR_ID. */
const DEFAULT_GOOGLE_CALENDAR_ID =
    "h55bus9k8a8buvp5kaole17udg@group.calendar.google.com";
...
/**
 * Calendario destino: prioriza GOOGLE_CALENDAR_ID; si falta, el calendario secundario personal.
 * Sin impersonación: la service account actúa como ella misma sobre un calendario compartido.
 */
function resolvePersonalCalendarId() {
    const fromEnv = String(process.env.GOOGLE_CALENDAR_ID || "")
        .replace(/[\r\n\s]+/g, "")
        .trim();
    return fromEnv || DEFAULT_GOOGLE_CALENDAR_ID;
}
...
function getGoogleCalendarClient() {
    ...
    // Solo client_email + private_key. Sin `subject` (no aplica a @gmail.com personal).
...
async function createGoogleCalendarMeetingEvent(meeting) {
    ...
    try {
        const calendarId = String(clientResult.calendarId || DEFAULT_GOOGLE_CALENDAR_ID).replace(
            /[\r\n\s]+/g,
            ""
        );

        // Sin organizer/creator: la API asigna el dueño del calendarId.
        const eventPayload = {
        ...
        throw new Error(
            `NUEVO ERROR API: ${error.message} - ID: [${clientResult.calendarId || DEFAULT_GOOGLE_CALENDAR_ID}] - Bot autenticado: [${credentials.client_email}]`
        );
    }
}
