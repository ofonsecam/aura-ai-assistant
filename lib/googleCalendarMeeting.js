...
/** Cuenta personal dueña del calendario: creador/organizador de las reuniones. */
const PERSONAL_GOOGLE_ACCOUNT = "fonseca.moreno.oscareduardo@gmail.com";
...
/**
 * ID del calendario personal = correo Gmail del dueño/organizador.
 * GOOGLE_CALENDAR_ID puede coincidir; si falta o difiere, se usa la cuenta personal.
 */
function resolvePersonalCalendarId() {
    const fromEnv = String(process.env.GOOGLE_CALENDAR_ID || "")
        .replace(/[\r\n\s]+/g, "")
        .trim();
    if (fromEnv && fromEnv.toLowerCase() === PERSONAL_GOOGLE_ACCOUNT.toLowerCase()) {
        return fromEnv;
    }
    return PERSONAL_GOOGLE_ACCOUNT;
}

function getGoogleCalendarClient() {
    const jsonEnv = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
    const calendarId = resolvePersonalCalendarId();

    // Validación estricta antes de proceder
    if (!jsonEnv) {
        throw new Error(
            `Error fatal: Variables de entorno faltantes. JSON_PROVISTO: ${!!jsonEnv}, CALENDAR_ID: ${calendarId}`
        );
    }
    ...
    const calendarApi = calendar({ version: "v3", auth: googleAuth });

    return {
        ok: true,
        calendarId,
        organizerEmail: PERSONAL_GOOGLE_ACCOUNT,
...
async function createGoogleCalendarMeetingEvent(meeting) {
    ...
    try {
        const organizerEmail = clientResult.organizerEmail || PERSONAL_GOOGLE_ACCOUNT;
        const cleanCalendarId = String(clientResult.calendarId || PERSONAL_GOOGLE_ACCOUNT).replace(
            /[\r\n\s]+/g,
            ""
        );
        ...
        const eventData = {
            ...
            // Evento individual en el calendario personal (sin invitados).
            organizer: {
                email: organizerEmail,
                self: cleanCalendarId.toLowerCase() === organizerEmail.toLowerCase(),
            },
            creator: {
                email: organizerEmail,
                self: cleanCalendarId.toLowerCase() === organizerEmail.toLowerCase(),
        ...
        throw new Error(
            `NUEVO ERROR API: ${error.message} - ID: [${clientResult.calendarId || PERSONAL_GOOGLE_ACCOUNT}] - Organizador: [${PERSONAL_GOOGLE_ACCOUNT}] - Bot autenticado: [${credentials.client_email}]`
        );
    }
}
