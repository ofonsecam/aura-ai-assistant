...
/**
 * Calendario destino: GOOGLE_CALENDAR_ID (env) o la cuenta personal @gmail.com.
...
function resolvePersonalCalendarId() {
    const fromEnv = String(process.env.GOOGLE_CALENDAR_ID || "")
        .replace(/[\r\n\s]+/g, "")
        .trim();
    return fromEnv || PERSONAL_GOOGLE_ACCOUNT;
}
...
async function createGoogleCalendarMeetingEvent(meeting) {
    ...
    try {
        const calendarId = String(clientResult.calendarId || PERSONAL_GOOGLE_ACCOUNT).replace(
            /[\r\n\s]+/g,
            ""
        );
        ...
        throw new Error(
            `NUEVO ERROR API: ${error.message} - ID: [${clientResult.calendarId || PERSONAL_GOOGLE_ACCOUNT}] - Bot autenticado: [${credentials.client_email}]`
        );
    }
}
