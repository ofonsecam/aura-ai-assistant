 ...
 * Iglesia (opcionales, select): Organización; Tipo Iglesia.
 */
const PROP_TASK_NAME = 'Name';
const PROP_TASK_ESTADO = 'Estado';
const PROP_TASK_FECHA = 'Fecha';
const PROP_TASK_AREA = 'Area';
const PROP_TASK_ORGANIZACION = 'Organización';
const PROP_TASK_TIPO_IGLESIA = 'Tipo Iglesia';
...
const CHURCH_ORGANIZACIONES = Object.freeze([
    'Obispado',
    'RP',
    'Seminarios',
...
const CHURCH_TIPOS = Object.freeze([
    'Reuniones',
    'Llamamientos',
    'Seguimientos',
    'Entrevistas',
...
/** Zona civil de Aura (Colombia; sin DST, offset fijo UTC-5). */
...
/** Opciones exactas del select Notion `Quien` (sin alias ni variaciones de mayúsculas). */
const TENSION_QUIEN_OPTIONS = Object.freeze(['Oscar', 'Yulis', 'Yulieth']);
/** Detección en consultas: nombres más largos primero (Yulieth vs Yulis). */
const TENSION_QUIEN_DETECT_ORDER = Object.freeze(['Yulieth', 'Oscar', 'Yulis']);
const TENSION_HISTORY_DAYS = 21;
const TENSION_TOP5_LIMIT = 5;
const TENSION_INVALID_FORMAT_MSG =
    '⚠️ Formato inválido. Usa: T/ <Oscar|Yulis|Yulieth> <Sistólica/Diastólica> (Ej: T/ Oscar 126/86)';
const TENSION_HISTORY_MISSING_QUIEN_MSG =
    '⚠️ Dime de quién es la tensión mi papacho: Oscar, Yulis o Yulieth.';
/** Atajos: `his tension Oscar`, `hist tension Yulis`, `tensión de Yulieth …`. */
const TENSION_HISTORY_CMD_RE =
    /^(?:his(?:t(?:orial)?)?\s+tensi[oó]n|tensi[oó]n(?:\s+de)?)\s+(oscar|yulis|yulieth)(?:\s+.*)?$/i;
const TENSION_HISTORY_PREFIX_RE =
    /^(?:his(?:t(?:orial)?)?\s+tensi[oó]n|tensi[oó]n(?:\s+de)?)\b/i;
/** Atajos: `top5 tension Oscar`, `top 5 tension de Yulis`. */
const TENSION_TOP5_CMD_RE = /^top\s*5\s+tensi[oó]n(?:\s+de)?\s+(oscar|yulis|yulieth)$/i;
const TENSION_TOP5_PREFIX_RE = /^top\s*5\s+tensi[oó]n/i;
const TENSION_TOP5_RANK_MARKERS = Object.freeze(['1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣']);
...
/**
 * Partes de reloj de pared en America/Bogota para un instante (Vercel = UTC).
 * @param {Date} [date]
 * @returns {{ ymd: string, hm: string, hms: string }}
 */
function getBogotaDateTimeParts(date = new Date()) {
    const instant = date instanceof Date && !Number.isNaN(date.getTime()) ? date : new Date();
    ...
    let hour = String(get('hour')).padStart(2, '0');
    if (hour === '24') hour = '00';
    const ymd = `${get('year')}-${String(get('month')).padStart(2, '0')}-${String(get('day')).padStart(2, '0')}`;
...
/**
 * Título + ISO de Notion para una toma de tensión (reloj de pared Bogotá).
 * @param {Date} [date]
 * @returns {{ title: string, iso: string, dateYmd: string }}
 */
function buildTensionNotionDatePayload(date = new Date()) {
    const { ymd, hm, hms } = getBogotaDateTimeParts(date);
    return {
        title: `${ymd} ${hm}`,
        iso: `${ymd}T${hms}${BOGOTA_OFFSET}`,
        dateYmd: ymd,
    };
}

/**
 * Rango de consulta: ahora Bogotá menos 21 días continuos.
 * @param {Date} [date]
 * @returns {{ fromIso: string, toIso: string, fromYmd: string, toYmd: string }}
 */
function getTensionHistoryRange(date = new Date()) {
    const to = buildTensionNotionDatePayload(date);
    const fromInstant = new Date(new Date(to.iso).getTime() - TENSION_HISTORY_DAYS * DAY_MS);
    const from = buildTensionNotionDatePayload(fromInstant);
    return { fromIso: from.iso, toIso: to.iso, fromYmd: from.dateYmd, toYmd: to.dateYmd };
}

/**
 * Marca de tiempo actual en Bogotá (GMT-5) para propiedad date de Notion con hora.
 * @returns {string} p. ej. 2026-04-21T20:30:45-05:00
 */
function getNowBogotaIsoForNotionDateTime() {
    return buildTensionNotionDatePayload(new Date()).iso;
}
...
function isIglesiaArea(area) {
    return toComparableKey(area) === toComparableKey(CHURCH_AREA);
}

function isDefaultPersonalArea(area) {
    return !String(area ?? '').trim() || toComparableKey(area) === toComparableKey('Personales');
}

function pickChurchSelectValue(raw, whitelist) {
    const key = toComparableKey(raw);
    if (!key) return '';
    const allowed = Array.isArray(whitelist) ? whitelist : [];
    const byKey = new Map(allowed.map((v) => [toComparableKey(v), v]));
    return byKey.get(key) || '';
}

function foldChurchDetectText(text) {
    return toComparableKey(text);
}

/**
 * NLP ligero: detecta Organización / Tipo Iglesia por palabras clave en el título.
 * Si hay coincidencia, también sugiere Area = Iglesia (sin forzar si el área explícita es otra).
 * @param {...string} texts
 * @returns {{ detected: boolean, organizacion: string, tipoIglesia: string }}
 */
function inferChurchTaskFields(...texts) {
    const t = foldChurchDetectText(texts.filter((x) => x != null && String(x).trim()).join(' '));
    if (!t) return { detected: false, organizacion: '', tipoIglesia: '' };

    let tipoIglesia = '';
    if (/\bentrevist(?:ar|as?)?\b/.test(t)) tipoIglesia = 'Entrevistas';
    else if (/\breuniones?\b|\bconsejos?\b/.test(t)) tipoIglesia = 'Reuniones';
    else if (/\bactividades?\b|\bsocial(?:es)?\b/.test(t)) tipoIglesia = 'Actividades';

    let organizacion = '';
    if (/\bprimaria\b/.test(t)) organizacion = 'Primaria';
    else if (/\bseminarios?\b/.test(t)) organizacion = 'Seminarios';
    else if (/\btemplo\b|\bhistoria familiar\b|\bindexar\b|\bfamilysearch\b/.test(t)) {
        organizacion = 'Templo e Historia Familiar';
    } else if (/\bescuela dominical\b|\bas\b/.test(t)) {
        organizacion = 'As';
    }

    return {
        detected: Boolean(organizacion || tipoIglesia),
        organizacion,
        tipoIglesia,
    };
}

const CHURCH_ORG_ALIASES = Object.freeze(['organizacion', 'Organizacion', 'Organización']);
const CHURCH_TIPO_ALIASES = Object.freeze(['tipoIglesia', 'TipoIglesia', 'Tipo Iglesia']);

function resolveOrganizacionFromTaskData(taskData, inferred) {
    const data = taskData && typeof taskData === 'object' ? taskData : {};
    for (const key of CHURCH_ORG_ALIASES) {
        const picked = pickChurchSelectValue(data[key], CHURCH_ORGANIZACIONES);
        if (picked) return picked;
    }
    return inferred || '';
}

function resolveTipoIglesiaFromTaskData(taskData, inferred) {
    const data = taskData && typeof taskData === 'object' ? taskData : {};
    for (const key of CHURCH_TIPO_ALIASES) {
        const picked = pickChurchSelectValue(data[key], CHURCH_TIPOS);
        if (picked) return picked;
    }
    return inferred || '';
}
...
async function createNotionTaskPage(taskData) {
    ...
    area = area.trim();
    const inferredChurch = inferChurchTaskFields(taskData.Name, name);
    const organizacion = resolveOrganizacionFromTaskData(taskData, inferredChurch.organizacion);
    const tipoIglesia = resolveTipoIglesiaFromTaskData(taskData, inferredChurch.tipoIglesia);
    if (inferredChurch.detected && (isDefaultPersonalArea(area) || isIglesiaArea(area))) {
        area = CHURCH_AREA;
    }
    if (isIglesiaArea(area)) area = CHURCH_AREA;
    if (/\b(URGENTE|YA|IMPORTANTE)\b/i.test(name)) {
        if (!name.startsWith('🚨')) name = '🚨 ' + name;
        if (isDefaultPersonalArea(area)) area = 'IA Dev';
    }
    ...
    if (isIglesiaArea(area)) {
        if (organizacion) {
            properties[PROP_TASK_ORGANIZACION] = { select: { name: organizacion } };
        }
        if (tipoIglesia) {
            properties[PROP_TASK_TIPO_IGLESIA] = { select: { name: tipoIglesia } };
        }
    }
...
/**
 * Valida Quién contra el select de Notion (`Oscar` | `Yulis` | `Yulieth`).
 * Solo acepta coincidencia exacta (sin alias, sin cambiar mayúsculas).
 * @param {string} raw
 * @returns {string|null}
 */
function normalizeTensionQuien(raw) {
    const value = String(raw || '').trim();
    if (TENSION_QUIEN_OPTIONS.includes(value)) return value;
    return null;
}
 ...
 * Título `YYYY MM DD` = `YYYY-MM-DD HH:mm` en reloj de pared Bogotá;
 * Fecha = ISO 8601 `YYYY-MM-DDTHH:mm:ss-05:00`;
 * Tension = rich_text; Quien = select Oscar|Yulis|Yulieth (exacto).
...
async function createNotionTensionPage({ quien, tension }) {
    ...
    const stamp = buildTensionNotionDatePayload(new Date());
    const properties = {
        [PROP_TENSION_TITLE]: { title: [{ text: { content: stamp.title } }] },
        [PROP_TENSION_FECHA]: { date: { start: stamp.iso } },
    ...
    try {
        ...
        if (res.ok) {
            ...
            return {
                ...
                id: data.id,
                url: data.url,
                dateYmd: stamp.dateYmd,
                dateStamp: stamp.title,
                iso: stamp.iso,
                quien: quienExact,
                tension: reading,
            };
...
/**
 * Detecta una etiqueta `Quien` en texto libre (insensible a mayúsculas).
 * @param {string} text
 * @returns {string|null}
 */
function detectTensionQuienInText(text) {
    const raw = String(text || '');
    for (const option of TENSION_QUIEN_DETECT_ORDER) {
        const re = new RegExp(`\\b${option}\\b`, 'i');
...
/**
 * Router híbrido de consultas de tensión: atajos cortos (regex) y frases naturales.
 * Top 5 se evalúa antes que historial para no colisionar con la palabra "tensión".
 * @param {string} text
 * @returns {{ ok: true, intent: 'history'|'top5', quien: string } | { ok: false, missingQuien?: boolean, intent?: 'history'|'top5' }}
 */
function parseTensionQuery(text) {
    const raw = String(text || '').replace(/\s+/g, ' ').trim();
    if (!raw || /^[tT]\//.test(raw)) return { ok: false };
    if (/^\S+\s+\d{2,3}\s*\/\s*\d{2,3}$/.test(raw)) return { ok: false };

    const top5Cmd = raw.match(TENSION_TOP5_CMD_RE);
    if (top5Cmd) {
        const quien = detectTensionQuienInText(top5Cmd[1]);
        if (quien) return { ok: true, intent: 'top5', quien };
    }
    if (TENSION_TOP5_PREFIX_RE.test(raw)) {
        const quien = detectTensionQuienInText(raw);
        if (quien) return { ok: true, intent: 'top5', quien };
        return { ok: false, missingQuien: true, intent: 'top5' };
    }

    const historyCmd = raw.match(TENSION_HISTORY_CMD_RE);
    if (historyCmd) {
        const quien = detectTensionQuienInText(historyCmd[1]);
        if (quien) return { ok: true, intent: 'history', quien };
    }
    if (TENSION_HISTORY_PREFIX_RE.test(raw)) {
        const quien = detectTensionQuienInText(raw);
        if (quien) return { ok: true, intent: 'history', quien };
        return { ok: false, missingQuien: true, intent: 'history' };
    }

    const hasTensionWord = /tensi[oó]n/i.test(raw);
    const hasHistorial = /historial|\bhis(?:t)?\b/i.test(raw);
    if (!hasTensionWord && !hasHistorial) return { ok: false };

    const quien = detectTensionQuienInText(raw);
    if (quien && hasTensionWord) return { ok: true, intent: 'history', quien };

    const looksLikeHistoryAsk =
        hasTensionWord || (hasHistorial && /tensi[oó]n|toma|semanas?|d[ií]as?/i.test(raw));
    if (looksLikeHistoryAsk) return { ok: false, missingQuien: true, intent: 'history' };
    return { ok: false };
}

/**
 * Interpreta consultas de historial (atajos + lenguaje natural).
 * @param {string} text
 * @returns {{ ok: true, quien: string } | { ok: false, missingQuien?: boolean }}
 */
function parseTensionHistoryQuery(text) {
    const parsed = parseTensionQuery(text);
    if (parsed.ok && parsed.intent === 'history') return { ok: true, quien: parsed.quien };
    if (!parsed.ok && parsed.missingQuien && parsed.intent === 'history') {
        return { ok: false, missingQuien: true };
    }
    return { ok: false };
}

/**
 * Interpreta `top5 tension <quien>` / `top 5 tension de <quien>`.
 * @param {string} text
 * @returns {{ ok: true, quien: string } | { ok: false, missingQuien?: boolean }}
 */
function parseTensionTop5Query(text) {
    const parsed = parseTensionQuery(text);
    if (parsed.ok && parsed.intent === 'top5') return { ok: true, quien: parsed.quien };
    if (!parsed.ok && parsed.missingQuien && parsed.intent === 'top5') {
        return { ok: false, missingQuien: true };
    }
    return { ok: false };
}
...
/**
 * @param {string} start
 * @returns {string} `DD/MM/YYYY HH:mm` en America/Bogota
 */
function formatBogotaDisplayFromNotionDate(start) {
    ...
    const { ymd, hm } = getBogotaDateTimeParts(instant);
...
/**
 * @param {string} raw
 * @returns {{ sis: number, dia: number } | null}
 */
function parseTensionMmHg(raw) {
    const m = String(raw || '').replace(/\s+/g, '').match(/^(\d{2,3})\/(\d{2,3})$/);
    if (!m) return null;
    return { sis: Number(m[1]), dia: Number(m[2]) };
}

/**
 * @param {{ tension: string }[]} readings
 * @returns {{ sys: number, dia: number, count: number } | null}
 */
function averageTensionReadings(readings) {
    const parsed = [];
    for (const row of readings || []) {
        const mm = parseTensionMmHg(row?.tension);
        if (mm) parsed.push(mm);
    }
    if (parsed.length < 2) return null;
    const sys = Math.round(parsed.reduce((acc, r) => acc + r.sis, 0) / parsed.length);
    const dia = Math.round(parsed.reduce((acc, r) => acc + r.dia, 0) / parsed.length);
    return { sys, dia, count: parsed.length };
}

/**
 * Ordena por sistólica y desempata por diastólica; descarta lecturas corruptas.
 * @param {{ tension: string, display?: string, iso?: string }[]} readings
 * @param {number} [limit]
 */
function rankTopTensionReadings(readings, limit = TENSION_TOP5_LIMIT) {
    const parsed = [];
    for (const row of readings || []) {
        const mm = parseTensionMmHg(row?.tension);
        if (!mm) continue;
        parsed.push({
            ...
            sis: mm.sis,
            dia: mm.dia,
            tension: `${mm.sis}/${mm.dia}`,
        });
    }
    parsed.sort((a, b) => b.sis - a.sis || b.dia - a.dia);
    return parsed.slice(0, Math.max(0, limit));
}

/**
 * @param {{ ok?: boolean, quien: string, fromYmd: string, toYmd: string, readings: { tension: string, display: string }[] }} result
 * @returns {string}
 */
function formatTensionHistoryTelegramMessage(result) {
    const quien = result?.quien || 'esa persona';
    const readings = Array.isArray(result?.readings) ? result.readings : [];
    if (!readings.length) {
        return `🩺 No hay tomas de tensión de ${quien} en los últimos ${TENSION_HISTORY_DAYS} días mi papacho.`;
    }
    const fromLabel = formatYmdToDisplay(result.fromYmd);
    const toLabel = formatYmdToDisplay(result.toYmd);
    const lines = [
        `🩺 Tensión de ${quien} — últimos ${TENSION_HISTORY_DAYS} días`,
        `📅 ${fromLabel} → ${toLabel}`,
        '',
    ];
    for (const row of readings) {
        const value = String(row.tension || '').trim() || '—';
        lines.push(`📅 ${row.display} | 🩸 ${value} mmHg`);
    }
    const avg = averageTensionReadings(readings);
    if (avg) {
        lines.push('', `📊 Promedio: ${avg.sys}/${avg.dia} mmHg (${avg.count} tomas)`);
    }
    return lines.join('\n');
}

/**
 * @param {{ ok?: boolean, quien: string, readings: { tension: string, display: string }[] }} result
 * @returns {string}
 */
function formatTensionTop5TelegramMessage(result) {
    const quien = result?.quien || 'esa persona';
    const readings = Array.isArray(result?.readings) ? result.readings : [];
    if (!readings.length) {
        return `🩺 No hay tomas válidas de tensión de ${quien} para armar el top 5 mi papacho.`;
    }
    const lines = [`🚨 Top 5 Tensiones más altas — ${quien}`, ''];
    readings.forEach((row, index) => {
        const marker = TENSION_TOP5_RANK_MARKERS[index] || `${index + 1}.`;
        const value = String(row.tension || '').trim() || '—';
        const when = row.display || '—';
        lines.push(`${marker} 🩸 ${value} mmHg | 📅 ${when}`);
    });
    return lines.join('\n');
}

/**
 * Cuerpo de `databases.query` para historial de 21 días (filtro + sort).
 * @param {string} quien
 * @param {string} fromIso
 */
function buildTensionHistoryQueryBody(quien, fromIso) {
    return {
        filter: {
            and: [
                { property: PROP_TENSION_QUIEN, select: { equals: quien } },
                { property: PROP_TENSION_FECHA, date: { on_or_after: fromIso } },
            ],
        },
        sorts: [{ property: PROP_TENSION_FECHA, direction: 'descending' }],
    };
}

/** Filtro solo por `Quien` (Top 5: el ranking se hace en memoria). */
function buildTensionQuienQueryBody(quien) {
    return {
        filter: { property: PROP_TENSION_QUIEN, select: { equals: quien } },
        sorts: [{ property: PROP_TENSION_FECHA, direction: 'descending' }],
    };
}

function extractTensionReadingFromPage(page) {
    const props = page?.properties || {};
    const tension = notionRichTextToPlain(props[PROP_TENSION_VALUE]?.rich_text) || '';
    const start = props[PROP_TENSION_FECHA]?.date?.start || '';
    return {
        tension: String(tension).replace(/\s+/g, '').trim(),
        iso: start,
        display: formatBogotaDisplayFromNotionDate(start),
    };
}

function resolveTensionQuienExact(quien) {
    return normalizeTensionQuien(quien) || detectTensionQuienInText(quien);
}

/**
 * Query paginada a DB_Tension. `queryBody` es filter+sorts (sin page_size).
 * @param {object} queryBody
 * @returns {Promise<{ ok: true, pages: object[] } | string>}
 */
async function queryNotionTensionDatabasePages(queryBody) {
    if (!notionTensionId) {
        return '❌ Falta NOTION_DB_TENSION_ID en el entorno (Vercel → Variables).';
    }
    if (!NOTION_UUID_RE.test(notionTensionId)) {
    ...
    try {
        for (;;) {
            ...
            if (start_cursor) body.start_cursor = start_cursor;
            const res = await fetch(`https://api.notion.com/v1/databases/${notionTensionId}/query`, {
                method: 'POST',
                headers: NOTION_HEADERS,
                body: JSON.stringify(body),
            });
...
/**
 * Consulta DB_Tension: lecturas de `quien` con Fecha on_or_after (ahora Bogotá − 21 días).
 * Equivale a `client.databases.query` con filter+sorts.
 * @param {string} quien
 * @returns {Promise<{ ok: true, quien: string, fromIso: string, toIso: string, fromYmd: string, toYmd: string, readings: { tension: string, iso: string, display: string }[] } | string>}
 */
async function queryNotionTensionLast21Days(quien) {
    const quienExact = resolveTensionQuienExact(quien);
    if (!quienExact) {
        return TENSION_HISTORY_MISSING_QUIEN_MSG;
    }
    const range = getTensionHistoryRange(new Date());
    const queried = await queryNotionTensionDatabasePages(
        buildTensionHistoryQueryBody(quienExact, range.fromIso)
    );
    ...
    return {
        ...
        toYmd: range.toYmd,
        readings: queried.pages.map(extractTensionReadingFromPage),
    };
}

/**
 * Top 5 lecturas más altas de `quien` (sistólica, desempate diastólica). Ranking en memoria.
 * @param {string} quien
 * @returns {Promise<{ ok: true, quien: string, readings: { tension: string, iso: string, display: string, sis: number, dia: number }[] } | string>}
 */
async function queryNotionTensionTop5(quien) {
    const quienExact = resolveTensionQuienExact(quien);
    if (!quienExact) {
        return TENSION_HISTORY_MISSING_QUIEN_MSG;
    }
    const queried = await queryNotionTensionDatabasePages(buildTensionQuienQueryBody(quienExact));
    if (typeof queried === 'string') return queried;
    const readings = rankTopTensionReadings(
        queried.pages.map(extractTensionReadingFromPage),
        TENSION_TOP5_LIMIT
    );
    return { ok: true, quien: quienExact, readings };
}
